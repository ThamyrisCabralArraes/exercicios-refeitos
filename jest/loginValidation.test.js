...
const { greetingMessage, loginErrorMessage, verifyCredentials } = require('./loginValidation.js');
...
describe('a função verifyCredentials()', () => {
  ...
  it('verifyCredentials() calls the correct function depending on the user and password input', () => {
    expect(verifyCredentials(user)).toBe('Pessoa usuária Bob não encontrada, tente novamente!');
  });
